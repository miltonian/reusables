...
.side_datecontainer, .side_titlecontainer {padding-left: 10px}
...
<script>
	...
	function populatesidetable(inputarray,adsetdict){
		...
		for( i=0;i<inputarray.length;i++ ){
			...
			var div = document.getElementById('side_cellleftdiv_'.concat(i));
	...
	function addSideTable(cellarray, cellsinrow, adsetdict ) 
{
    ...
    for (a = 0; a < cellarray.length; a++) {
        	...
        	if( (multiplethree == 5 || a==2) && numberofads<4 && adsetdict.length>0){
...
function setupSideTableCell(inputdict, theindex, thisdiv) 
{
	...
	leftdiv.id = 'side_celltopdiv_'.concat(theindex);
	leftdiv.style.backgroundImage = backgroundimagestring2;
	...
	rightdiv.appendChild(titlecontainer);
	...
	rightdiv.appendChild(datecontainer);
