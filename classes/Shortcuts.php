		...
		if( $file['name'] != null && $file['name'] != "" ){
			...
			if (($file["size"] < 20000000) && in_array($extension, $allowedExts)){
				...
				if ($file["error"] > 0)
				{
					...
					if (file_exists("vendor/miltonian/custom/uploads/" . $newname)){
						$uploadOk = 0;
					}else{
						move_uploaded_file($file["tmp_name"], "vendor/miltonian/custom/uploads/" . $newname);
						if(filesize('vendor/miltonian/custom/uploads/' . $newname) > 3000000){
							$scalefactor = round(100*(3000000 / filesize("vendor/miltonian/custom/uploads/" . $newname)));
							$result = self::convertImage('vendor/miltonian/custom/uploads/' . $newname, 'vendor/miltonian/custom/uploads/thumbs_small/' . $newname, $scalefactor);
						}else{
							$result = self::convertImage('vendor/miltonian/custom/uploads/' . $newname, 'vendor/miltonian/custom/uploads/thumbs_small/' . $newname, 100);
						}
						$uploadOk = 1;
					}
				}
